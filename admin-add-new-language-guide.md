...
- 源快照列表。
- 翻译文章列表和翻译矩阵。
- 单篇源文章语言版本页，当前源文章语言不会作为目标语言行显示。
- 翻译文章编辑器。
- 翻译任务列表。
- 关联内容翻译列表。
- 多语言作者、分类、标签、地点、关联内容源列表和语言版本页。
- 多语言媒体库源媒体列表和媒体语言版本页。
- 导航、侧边栏、Banner 管理。
- 专有名词翻译页面。
...
- 博客端语言启用状态：新增语言默认关闭；内容和站点配置准备完成后，再在 Admin 多语言站点配置中手动开启。该开关只影响 Blog 前台和 Blog API，不影响 Admin 继续维护内容。
- AI 设置：新增语言后，必须把 7 条流程逐条检查完，确认新语言在对应的按语言提示词映射里已经出现；如果该流程没有额外要求，也要保留该语言对应的空字符串，不要漏掉该语言键。各流程通用要求分别放在对应的默认提示词字段里。

| 流程                      | 必查字段                              | 检查要求                   |
| ------------------------- | ------------------------------------- | -------------------------- |
| 主翻译 AI                 | `mainTranslationLanguagePrompts`      | 必须出现新语言 code 对应项 |
| 校验 AI                   | `verificationLanguagePrompts`         | 必须出现新语言 code 对应项 |
...
- 源文章导入页可以选择新语言作为目标语言。
- 翻译文章列表语言矩阵显示新语言状态。
- 单篇语言版本页在源文章语言之外出现新语言行，并能创建翻译。
- 翻译编辑器能打开新语言文章。
- 关联翻译列表能筛选新语言。
- 多语言作者、分类、标签、地点、关联内容源列表语言矩阵显示新语言状态，语言版本页出现新语言行。
- 多语言作者、分类、标签、地点、关联内容语言版本页保留详情、编辑、AI 翻译、同步快照、AI 跳过等旧操作。
- 多语言媒体库源媒体列表语言矩阵显示新语言状态，媒体语言版本页能筛选和操作新语言媒体。
