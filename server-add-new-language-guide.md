...
> 关于多语言翻译任务拆分：「源文生成并 AI 翻译」在目标语言多于 1 种时，会自动按家族结构
> 拆成顺序执行的子任务（每篇文章一个父任务，父任务下挂「名词整理 → 各语言翻译 → 封面图整理」
> 的有序子任务），以规避 MongoDB 单文档 16MB 限制。子任务数量由 `target.languageCodes`
> 派生，而 `target.languageCodes` 来源于语言配置。因此新增语言时无需任何额外手顺：家族会
> 自动多出一个「单语言翻译」子任务，名词整理与封面图整理子任务也会自动覆盖新语言。无需修改
> 拆分相关代码。
