# GET /api/multilingual-blog/movie/list

## 概述

- 接口类型：多语言 Blog API
- 说明：分页获取电影列表。
- 访问路径：`/api/multilingual-blog/movie/list`
- 上游路径：同访问路径
- 控制器：`server/api/blog/movie/getMovieList.js`
- 前端封装：`blog/app/api/movie.js#getMovieListApi`

## 请求

- Method：`GET`
- URL：`/api/multilingual-blog/movie/list`
...
| `year` | Query | number | 否 | 上映年份；会校验年份有效性。 |
