...
| 服务端博客 API               | `server/routes/blog.js`、`server/api/blog/*`                                                                       | 源项目有 options、navi、post、banner、sidebar、comment、sort、archive、like/share、link、bangumi、movie、tag、mappoint、game、book、attachment、event、log、trend、vote。多语言站自有内容接口需要语言 code 过滤；源站接管的 options、comment、like/share 等接口由 blog 端直接请求源站接口。                |
...
| 友链                         | `links.js`                                                                                                         | 用户明确友链不需要多语言。多语言后台不提供本地友链配置；博客端友链列表直接请求源站 `/api/blog/link/list`。                                                                                                                                                                                                 |
| 评论                         | `comments.js`、`server/api/blog/comment/*`                                                                         | 完全由源站接管，blog 端直接请求源站评论接口；多语言 server 不反代、不写本地 comments、不提供评论后台。前端评论 UI 文案仍走语言包。                                                                                                                                                                         |
| 点赞                         | `postLikeLogs.js`、`commentLikeLogs.js`、`server/api/blog/postLikeLog/*`、`server/api/blog/commentLikeLog/*`       | 完全由源站接管，blog 端直接请求源站点赞接口；多语言 server 不写本地点赞日志，多语言后台不提供点赞统计管理。                                                                                                                                                                                                |
...
- 源站已接管接口继续保持源站路径，例如 `/api/blog/options`、`/api/blog/comment/*`、`/api/blog/post/like/log`、`/api/blog/comment/like/log` 等，由 blog 端直接请求。
...
- 评论相关请求由 blog 端直接请求源站 `/api/blog/comment/*`，不经过多语言 server，不写入本地 comments。
- 点赞相关请求由 blog 端直接请求源站点赞接口，不经过多语言 server，不写入本地点赞日志。
- 非多语言配置由 blog 端直接请求源站 `/api/blog/options` 等源站接口，不在多语言 server 代理和缓存。

Blog API 客户端拆分为两个实例：

| 文件                           | baseURL                  | 使用范围                                                                                                                                            |
| ------------------------------ | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `blog/app/api/source.js`       | `/api/blog`              | `options`、`comment/*`、`post/like/log`、`post/like/log/list`、`comment/like/log`、`comment/like/log/list`、`link/list`。                           |
